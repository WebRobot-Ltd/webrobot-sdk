...
This compiles the client into a jar located at `./target/WebRobotCli-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'WebRobot.Cli.Sdk:WebRobotCli:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>WebRobot.Cli.Sdk</groupId>
        <artifactId>WebRobotCli</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/WebRobotCli-1.0-SNAPSHOT.jar`.
...
```java
WebRobotCliClientBuilder builder = WebRobotCli.builder();
```
...
```java
WebRobotCli client = WebRobotCli.builder()
...
```java
WebRobotCli client = WebRobotCli.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`WebRobotCli`) is created for all actions (resource + method) in your API.
...
```java
WebRobotCli client = WebRobotCli.builder().build();
Add_scriptResult result = client.add_script(new Add_scriptRequest());
```
...
```java
WebRobotCli client = WebRobotCli.builder().build();
client.add_script(new Add_scriptRequest().sdkRequestConfig(
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of WebRobotCliException.
```java
try {
    client.add_script(...);
} catch(WebRobotCliException e) {
   // All service exceptions will extend from WebRobotCliException.
   // Any unknown or unmodeled service exceptions will be represented as a WebRobotCliException.
...
## Retries
Out of the box, the generated client retries on throttling errors (HTTP status code 429) and connection exceptions. If a different retry policy is desired, a WebRobot.Cli.Sdk.custom one can be set via the client builder.

The easiest way to create a WebRobot.Cli.Sdk.custom retry policy is to use the RetryPolicyBuilder. It provides a declarative API to specify when to retry.
...
// Using a WebRobot.Cli.Sdk.custom retry policy via the builder
WebRobotCli client = WebRobotCli.builder()
