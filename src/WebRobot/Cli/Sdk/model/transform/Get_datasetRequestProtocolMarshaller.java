...
public class Get_datasetRequestProtocolMarshaller implements Marshaller<Request<Get_datasetRequest>, Get_datasetRequest> {
    ...
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().protocol(Protocol.API_GATEWAY)
            .requestUri("/test/webrobot/api/datasets/{projectId}/{botId}/{datasetId}").httpMethodName(HttpMethodName.GET).hasExplicitPayloadMember(false)
            .hasPayloadMembers(false).serviceName("WebRobotCliSdk").build();

    private final ApiGatewayProtocolFactoryImplTest protocolFactory;

    public Get_datasetRequestProtocolMarshaller(ApiGatewayProtocolFactoryImplTest protocolFactory) {
